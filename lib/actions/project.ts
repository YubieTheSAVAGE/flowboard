...
export async function updateProject(id: string, prevState: UpdateProjectFormState | undefined, formData: FormData) {
    const session = await verifySession();
    if (!session?.userId) {
        return {
            message: "Unauthorized",
    ...
    const { name, description } = validatedFields.data;
    const project = await updateProjectDal(id, name, description);
    if (!project) {
        return {
